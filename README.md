...
Template vierge pour tester des interfaces PWA mobile-first, le mode standalone iOS, un mécanisme de mise à jour robuste et une authentification applicative intégrée dans un conteneur LXC Proxmox avec Docker et Docker Compose.
...
> Les identifiants `admin` / `admin` présents dans `.env.example` sont uniquement destinés au test local. Changez `ADMIN_PASSWORD` et `SESSION_SECRET` avant toute exposition réseau.

## Authentification et gestion des utilisateurs

- Un login est obligatoire avant d'accéder à l'application.
- La session est stockée dans un cookie `HttpOnly`, signée par `SESSION_SECRET`, avec expiration `SESSION_TTL_HOURS`.
...
- Dans `Paramètres`, un admin peut activer le mode admin local pour afficher `Administration → Gestion des utilisateurs`.
